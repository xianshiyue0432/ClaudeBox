...
export default function ChatPanel({ claudeAvailable }: ChatPanelProps) {
  ...
  const handleSend = useCallback(
    async (content: string, attachments?: Attachment[]) => {
      ...
      // Resolve creds the same way sendMessage does (per-model or global fallback)
      const creds = resolveModelCreds(effectiveModel || "", settings.models, settings.apiKey, settings.baseUrl);
      if (!creds.apiKey) {
      ...
      try {
        ...
        // creds already resolved above during validation
